...
The data here summarizes the number of accidents in 6 areas from 2005 to 2019 (15 years in total)， therefore the processed data is a $6 \times 15$ matrix.
```{python}
...
First, we could calcualte the mean accident rate in each area, which turns out to be approximately $25$.
```{python}
...
Besides, we implemented three sets of priors for sensitivity analysis (Section 4.5) and prepared log likelihood for cross validation.
```{python}
...
We define a common function to test the three different stan models, and for simplicity we just report the estimated value of the key parameters $\alpha$, $\beta$ and $\sigma$.
```{python}
