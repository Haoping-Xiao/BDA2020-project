...
In our case, $\hat{R}$ is 1.0 a $W$ and $B$ should be approximately equal, indicating that the chains have explored and converged to similar areas in the posterior distribution.
...
Also we use $R^2$[3] as a metric to measure the performance of our models. 
...
And the hierarchical model is slightly better than the separate model, with higher $R^2$ score and lower variance.

Now, we prefer hierarchical model to separate model since it is more reliable (discussed in section 4.2) and has better predictive performance.
```{python}
...
In this subsection, we manage to test sensitivity of our models to different prior distribution. Our models require slope and intercept prior information. Here, we treat prior distribution described in section 3 as default prior distribution, **which is based on our observation and assumption that the mean car accident rates changes less than 50 % in one year.** Then, we also apply non-informative prior distribution (uniform prior distribution) and another weakly prior similar with default prior, but it has larger variance.

As can be seen from the forest plots, our models are robust with different prior choices, because there is no clear deviation in the posterior distributions with different prior. In addition, Rhat (all values are equal to 1) and ESS values indicates models can be converged with different prior settings.
```{python}
data_dict = dict()
...
From the result of cross validation, we conclude pooled model is the worst model. 
The separate model has slightly larger elpd_loo value while the hierarchical model is more reliable.

After checking posterior predictive distribution, we further confirm that the pooled model is the worst model. 
The separate model and the hierarchical model fit the data well and give stable prediction.
...
Based on these results, it is clear that the hierarchical model is reliable and has good predictive performance. 
Therefore, we adopt **the hierarchical model** as the optimal model to analyze our problem, which is to predict car accident rate in different areas in UK. 
